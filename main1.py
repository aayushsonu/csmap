...
class PassGenerator:
    def __init__(self,l):
        ...
        I hope you dont mind it''','yellow'))
            print(colored("".join(random.sample(self.s, 8)),'green',attrs=['bold','blink']))
        elif self.passlen > 94:
            print(colored(f"Where to use {self.passlen} letters password LOL",'yellow'))
            print(colored(f"If you really want to create a password of {self.passlen} letters then Enter",'blue'))
            typed_string = input("")
            x = self.passlen // 94
            y = self.passlen % 94
            print(colored(("".join(random.sample(self.s, 94))) * x + ("".join(random.sample(self.s, y))),'green',attrs=['bold']))
        else:
            print(colored("Yup! Good choice of password length..!!",'yellow'))
            print(colored("".join(random.sample(self.s, self.passlen)),'green',attrs=['bold']))
...
class PortScanner:
    ...
    def portScan(tgtHost,tgtPorts):
        ...
        for tgtPort in tgtPorts:
            t=Thread(target=PortScanner.connScan, args=(tgtHost,int(tgtPort)))
            t.start()
...
if __name__ == "__main__":
    ...
    try:
        ...
        if fPlace == '-c' or fPlace=='-h' or fPlace=='--help':
            ...
            print(colored(f"Caeser Cipher Text: \'{val}\' ",'green',attrs=['bold']))
